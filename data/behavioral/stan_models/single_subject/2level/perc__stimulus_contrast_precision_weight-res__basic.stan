...
transformed parameters {
  ...
  vector<lower=0>[N_sess] zeta_sess;
  ...
  zeta_sess = exp(mu_zeta + sigma_zeta * zeta_eta);
...
model {
  ...
  target += std_normal_lpdf(mu_zeta);
  target += std_normal_lpdf(sigma_zeta);

  // Priors
  target += std_normal_lpdf(omega2_eta);
  target += std_normal_lpdf(zeta_eta);
