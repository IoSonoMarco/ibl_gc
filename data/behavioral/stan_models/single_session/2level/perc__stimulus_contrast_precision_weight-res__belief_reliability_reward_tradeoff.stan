functions {
  ...
  vector response_model(
    ...
    for (n in 1:N) {
      ...
      theta = beta_prior*mu2_hat[n] + beta_sens*sensory_evidence + beta_reward*z_rewarded[n] + beta_unreward*z_unrewarded[n];
...
model {
  ...
  target += std_normal_lpdf(beta_reward);
  target += std_normal_lpdf(beta_unreward);
...
generated quantities {
  ...
  vector[N_obs] reward_reg;
  vector[N_obs] unreward_reg;
  ...
  for (n in 1:N_obs) {
      ...
      reward_reg[n] = beta_reward*z_rewarded[n];
      unreward_reg[n] = beta_unreward*z_unrewarded[n];
    }
}